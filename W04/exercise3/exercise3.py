...
# Inputs and expected outputs for the XOR gate
inputs = [[0, 0], [0, 1], [1, 0], [1, 1]]
inputsArray = np.array(inputs)
expected_output = np.array([0, 1, 1, 0])  # XOR gate outputs
...
# Check if the perceptron learned XOR
print("\nError results:")
for expected, test_input in zip(expected_output, inputsArray):
    output = activation(np.dot(np.array([w_1, w_2]), test_input) - theta)
    print(f"Input: {test_input}, Expected: {expected}, Output: {output}")


# Perceptron cant learn an XOR gate, would need to use MLP 